...
sendFeedback = function(topic,learningResource,comment){
  var vars = window.location.search.substring(1).split('&');
  var params = {}
  for (var i=0;i<vars.length;i++) {
    var pair = vars[i].split("=");
...
getItemFromTopicLRTypeWithZPDAlgorithm = function(topicLRType, kcScores,mappingsAndDifficulties, recommendationsArr){
  ...
  if(topicLRType.indexOf(TS.toLowerCase()) != -1){
    ...
    var items = getItemsViaZPDAlgorithm(1,items,kcScores,mappingsAndDifficulties);
...
getItemsViaZPDAlgorithm = function(numItemsToGet,items,kcScores,mappingsAndDifficulties){
  log("getItemsViaZPDAlgorithm");
  var itemScores = calcScoresForZPDAlgorithm(items,kcScores,mappingsAndDifficulties);
...
calcScoresForZPDAlgorithm = function(items, kcScores,mappingsAndDifficulties){
